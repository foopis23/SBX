...
export const Area3D = trait<
  () => {
    ...
    collisions: Record<
      number,
      { otherEntity: Entity; otherCollider: RAPIER.Collider }
    >;
...
export function rigidbodyLifecycleSystem({ game }: { game: Game }) {
  ...
  game.world.query(rigidbodyRemoveQuery(RigidBody3D)).forEach((entity) => {
    ...
    const body = physicsWorld.bodies
      .getAll()
      .find((body) => (body.userData as { entity: Entity }).entity === entity);
    if (!body) {
  ...
  game.world
    ...
    .updateEach(([transform, rigidbody]) => {
      ...
      rigidbody.setRotation(
        { x: rotation.x, y: rotation.y, z: rotation.z, w: 1 },
        false,
      );
    });
};
...
export const syncTransformToArea3DSystem: System = ({ game }) => {
  game.world.query(Transform, Area3D).updateEach(([transform, area]) => {
    const position = transform.position;
    const rotation = transform.rotation;
    area.collider.setTranslation({
      x: position.x,
      y: position.y,
      z: position.z,
    });
    area.collider.setRotation({
      x: rotation.x,
      y: rotation.y,
      z: rotation.z,
      w: 1,
...
export function physicsSystem({ game }: { game: Game }) {
  ...
  const colliderMap = new Map<
    number,
    { collider: RAPIER.Collider; entity: Entity }
  >();
  ...
  eventQueue.drainCollisionEvents((colliderA, colliderB, started) => {
    ...
    if (started) {
      if (areaA) {
        areaA.collisions[b.collider.handle] = {
      ...
      if (areaB) {
        areaB.collisions[a.collider.handle] = {
          otherEntity: a.entity,
          otherCollider: a.collider,
        };
