...
function createMeshFromCollider(
  collider: Physics.RAPIER.Collider,
  ...
  switch (collider.shape.type) {
    case Physics.RAPIER.ShapeType.Ball:
      return new Graphics.THREE.Mesh(
        new Graphics.THREE.SphereGeometry(
          (collider.shape as Physics.RAPIER.Ball).radius,
        ),
    ...
    case Physics.RAPIER.ShapeType.Cuboid: {
      ...
      return new Graphics.THREE.Mesh(
        new Graphics.THREE.BoxGeometry(
          halfExtents.x * 2,
          halfExtents.y * 2,
          halfExtents.z * 2,
        ),
 ...
 * @returns
 */
export function createRigidBodyDebugMesh(
  rigidbody: Physics.RAPIER.RigidBody,
  options: { color?: number } = {},
) {
...
export function createAreaDebugMesh(
  area: Physics.RAPIER.Collider,
  options: { color?: number } = {},
) {
  const { color = 0xffff00 } = options;
