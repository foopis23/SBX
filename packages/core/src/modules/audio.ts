...
export class AudioBusNotFoundError extends Error {
	constructor(name: string) {
		super(`Bus "${name}" not found. Make sure to set the bus to "master" or create the bus through the AudioBusManager.`)
	}
}
...
export function routeToBus(audio: Graphics.THREE.Audio | Graphics.THREE.PositionalAudio, bus: AudioBus) {
...
export const syncTransformToPositionalAudioSystem: System = ({ game }) => {
	game.world.query(Transform, AudioSource).updateEach(([transform, source]) => {
		...
		source.audio.position.set(transform.position.x, transform.position.y, transform.position.z)
		source.audio.rotation.set(transform.rotation.x, transform.rotation.y, transform.rotation.z)
	})
}
