...
export class Input {
	private keysPressed: Record<string, { pressed: boolean; justPressed: boolean; justReleased: boolean }> = {};

	constructor() {
		window.addEventListener('keydown', (event) => {
			if (!this.keysPressed[event.code]?.pressed) {
				this.keysPressed[event.code] = { pressed: true, justPressed: true, justReleased: false };
			}
		});
		window.addEventListener('keyup', (event) => {
			this.keysPressed[event.code] = { pressed: false, justPressed: false, justReleased: true };
		});
	}
